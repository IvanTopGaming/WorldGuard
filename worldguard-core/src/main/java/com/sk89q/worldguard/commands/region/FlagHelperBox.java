...
class FlagHelperBox extends PaginationBox {
    ...
    FlagHelperBox(World world, ProtectedRegion region, RegionPermissionModel perms) {
        super("Флаги для " + region.getId(), "/rg flags -w \"" + world.getName() + "\" -p %page% " + region.getId());
        this.world = world;
        this.region = region;
        this.perms = perms;
    }
        ...
        if (flag.usesMembershipAsDefault()) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Это специальный флаг, который по умолчанию разрешён для участников и запрещён для остальных"))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
        if (flag == Flags.PASSTHROUGH) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Это специальный флаг, который переопределяет проверки на строительство. (Не связан с перемещением!)"))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
    ...
    private <V> void appendValueChoices(TextComponent.Builder builder, Flag<V> flag, Iterator<V> choices, @Nullable String suggestChoice) {
        ...
        while (choices.hasNext()) {
            ...
            if (maySet) {
                if (isExplicitSet) {
                    ...
                    hoverTexts.add(TextComponent.of("Установка этого флага может привести к нежелательным последствиям.", TextColor.RED)
                            .append(TextComponent.newline())
                            .append(TextComponent.of("Пожалуйста, прочитайте документацию и установите этот флаг вручную, если вы действительно этого хотите."))
                            .append(TextComponent.newline())
        ...
        if (suggestChoice != null && perms.maySetFlag(region, flag)) {
            builder.append(TextComponent.of(suggestChoice, TextColor.DARK_GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Нажмите, чтобы установить пользовательское значение", TextColor.GOLD)))
    ...
    private <V> Component getToolTipHint(V defVal, V currVal, boolean inherited) {
        ...
        if (inherited) {
            if (currVal == defVal) {
                valType = TextComponent.of("Унаследовано и ")
                        .append(TextComponent.of("по умолчанию")
                                .decoration(TextDecoration.UNDERLINED, true))
                        .append(TextComponent.of(" значение"));
            } else {
                valType = TextComponent.of("Унаследованное значение");
            }
        } else {
            if (currVal == defVal) {
                valType = TextComponent.empty()
                        .append(TextComponent.of("Значение по умолчанию")
                                .decoration(TextDecoration.UNDERLINED, true))
    ...
    private void appendLocationFlagValue(TextComponent.Builder builder, LocationFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, "значение не задано (координаты)", null);
            } else {
    ...
    private <V extends Number> void appendNumericFlagValue(TextComponent.Builder builder, NumberFlag<V> flag) {
        ...
        appendValueChoices(builder, flag, (Iterator<V>) choices.iterator(), choices.isEmpty() ? "значение не задано (число)" : "[своё]");
    }
    ...
    private void appendStringFlagValue(TextComponent.Builder builder, StringFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, "значение не задано (строка)", null);
            } else {
