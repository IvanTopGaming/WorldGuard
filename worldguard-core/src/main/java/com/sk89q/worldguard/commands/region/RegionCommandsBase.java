...
class RegionCommandsBase {
    ...
    protected static World checkWorld(CommandContext args, Actor sender, char flag, boolean allowWorldEditOverride) throws CommandException {
        if (args.hasFlag(flag)) {
            ...
            if (sender instanceof LocalPlayer) {
                ...
                throw new CommandException("Пожалуйста, укажите мир с помощью -" + flag + " world_name.");
            }
        }
    }
    ...
    protected static String checkRegionId(String id, boolean allowGlobal) throws CommandException {
        if (!ProtectedRegion.isValidId(id)) {
            throw new CommandException(
                    "Имя региона '" + id + "' содержит недопустимые символы.");
        }
    ...
    protected static ProtectedRegion checkRegionStandingIn(RegionManager regionManager, LocalPlayer player, boolean allowGlobal, String rgCmd) throws CommandException {
        ...
        if (set.size() == 0) {
            if (allowGlobal) {
                ...
                player.printDebug("Вы не находитесь ни в одном регионе. Используется глобальный регион этого мира.");
                return global;
            }
            throw new CommandException(
                    "Вы не находитесь в регионе. Укажите ID, если хотите выбрать конкретный регион.");
        } else if (set.size() > 1) {
            boolean first = true;
            ...
            throw new CommandException("Вы находитесь в нескольких регионах (выберите один).");
        }
    ...
    protected static Region checkSelection(Actor actor) throws CommandException {
        ...
        try {
            ...
            throw new CommandException("Пожалуйста, сначала выделите область. Используйте WorldEdit для выделения! (см.: https://worldedit.enginehub.org/ru/latest/usage/regions/selections/).");
        }
    }
    ...
    protected static RegionManager checkRegionManager(World world) throws CommandException {
        if (!WorldGuard.getInstance().getPlatform().getGlobalStateManager().get(world).useRegions) {
            throw new CommandException("Поддержка регионов отключена в целевом мире. Её можно включить для каждого мира в конфигурационных файлах WorldGuard. Возможно, потребуется перезапустить сервер.");
        }

        RegionManager manager = WorldGuard.getInstance().getPlatform().getRegionContainer().get(world);
        if (manager == null) {
            throw new CommandException("Не удалось загрузить данные регионов для этого мира. Попросите администратора сервера посмотреть логи для выяснения причины.");
        }
        return manager;
    }
    ...
    protected static ProtectedRegion checkRegionFromSelection(Actor actor, String id) throws CommandException {
        ...
        if (selection instanceof Polygonal2DRegion) {
            ...
            throw new CommandException("Извините, вы можете использовать только кубоиды и полигоны для регионов WorldGuard.");
        }
    }
    ...
    protected static void warnAboutSaveFailures(Actor sender) {
        ...
        if (!failures.isEmpty()) {
            ...
            sender.print(TextComponent.of("(Внимание: Фоновое сохранение данных регионов не работает для этих миров: " + failingList + ". Ваши изменения теряются. Смотрите лог сервера для подробностей.)", TextColor.GOLD));
        }
    }
    ...
    protected static void informNewUser(Actor sender, RegionManager manager, ProtectedRegion region) {
        if (manager.size() <= 2) {
            sender.print(SubtleFormat.wrap("(Этот регион теперь ЗАЩИЩЁН от изменений другими игроками. Не хотите этого? Используйте ")
    ...
    protected static boolean checkSpawnOverlap(Actor sender, World world, ProtectedRegion region) {
        ...
        if (spawn != null) {
            if (!spawn.getIntersectingRegions(ImmutableList.of(region)).isEmpty()) {
                sender.print(ErrorFormat.wrap("Внимание!")
                        .append(TextComponent.of(" Этот регион пересекается со спавн-защитой ванильного сервера. WorldGuard не может это переопределить, и только операторы сервера смогут взаимодействовать с этой областью.", TextColor.WHITE)));
                return true;
            }
        }
        return false;
    ...
    protected static void setPlayerSelection(Actor actor, ProtectedRegion region, World world) throws CommandException {
        ...
        if (selector != null) {
            ...
            throw new CommandException("Невозможно выбрать этот регион! Тип региона '" + region.getType().getName() + "' не может быть выбран.");
        }
    }
