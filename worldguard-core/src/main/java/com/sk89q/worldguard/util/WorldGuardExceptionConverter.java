...
public class WorldGuardExceptionConverter extends ExceptionConverterHelper {
    ...
    public void convert(NumberFormatException e) throws CommandException {
        ...
        if (matcher.matches()) {
            throw newCommandException("Ожидалось число; получена строка \"" + matcher.group(1)
                    + "\".", e);
        } else {
    ...
    public void convert(StorageException e) throws CommandException {
        ...
        throw newCommandException("Не удалось загрузить/сохранить данные регионов: " + e.getMessage(), e);
    }

    @ExceptionMatch
    public void convert(RejectedExecutionException e) throws CommandException {
        throw newCommandException("В данный момент слишком много задач в очереди. Используйте /wg running для просмотра очереди и выполняющихся задач.", e);
    }
